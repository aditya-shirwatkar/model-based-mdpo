...
def train(env_id, num_timesteps, seed, sgd_steps):
    """
    ...
    with tf_util.single_threaded_session():
        ...
        log_path = './experiments/'+str(env_id)+'./OURS-LOADED/noent_klcoeffanneal_samesgdsteps'+str(sgd_steps)+'_longer_wgae0.95_exp2_mpicpu_testkl-2_'+'_'+str(seed)
        log = True
        if log:
            if rank == 0:
        ...
        model = MDPO(MlpPolicy, env, timesteps_per_batch=2048, max_kl=0.01, cg_iters=10, cg_damping=0.1, entcoeff=0.0,
                     gamma=0.99, lam=0.95, vf_iters=5, vf_stepsize=1e-3, verbose=1, seed=seed, sgd_steps=sgd_steps, method="multistep-SGD")
...
def main():
    ...
    train(args.env, num_timesteps=args.num_timesteps, seed=args.run, sgd_steps=args.sgd_steps)
